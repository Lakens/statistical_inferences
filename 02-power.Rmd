...
# Justifying the Number of Observations {#power}
...
```{r, power-effect1, fig.margin=TRUE, echo=FALSE, fig.cap="Null and alternative distribution, assuming d = 0.5, alpha = 0.05, and N = 15 per group."}
N <- 15
d <- 0.5
p_upper <- 0.05
ncp <- (d * sqrt(N / 2)) # Calculate non-centrality parameter d
...
```{r gcrit1, fig.margin=TRUE, echo=FALSE, fig.cap="The critical *t*-value is provided by G\\*Power software."}
...
```{r gcrit2, fig.margin=TRUE, echo=FALSE, fig.cap="The critical r is provided by G\\*Power software."}
...
```{r, power-3, fig.margin=TRUE, echo=FALSE, fig.cap="Null and alternative distribution, assuming d = 0.5, alpha = 0.05, and N = 86 per group."}
N <- 86
d <- 0.5
p_upper <- 0.05
ncp <- (d * sqrt(N / 2)) # Calculate non-centrality parameter d
...
The output tells us we need to collect 85.03 observations. Because observations do not come in decimals (we can hardly cut 0.03 off a participant) we need to collect 86 observations. We could also get this answer through simulations. 
...
It takes time to learn to use this software correctly. You will need to sit down for a few hours and go through vignettes or read the accompanying publications, before you know what to do. In our online manual for Superpower, we compare Superpower against G\*Power, `pwr`, `pwr2ppl` and MorePower. 

### Types of power analysis {#typespower}
...
## Observed power, and what to do if your editor asks for post-hoc power analyses {#observedpower}
...
```{r obspow1, fig.margin=TRUE, echo=FALSE, fig.cap="Distribution of *p*-values plotted against observed power with low power."}
knitr::include_graphics("images/obspow1.png")
```
...
```{r obspow2, fig.margin=TRUE, echo=FALSE, fig.cap="Distribution of *p*-values plotted against observed power with high power."}
knitr::include_graphics("images/obspow2.png")
```
...
```{r obspow3, fig.margin=TRUE, echo=FALSE, fig.cap="Distribution of *p*-values plotted against observed power showing the correspondence of p = 0.05 and 50% power."}
knitr::include_graphics("images/obspow3.png")
```
