...
# What is power analysis? {#power}

## Statistical power and Type 2 error rates
...
```{r, power-effect1, fig.margin=TRUE, echo=FALSE, fig.cap="Null and alternative distribution, assuming d = 0.5, alpha = 0.05, and N = 86 per group."}
N <- 86
d <- 0.5
p_upper <- 0.05
ncp <- (d * sqrt(N / 2)) # Calculate non-centrality parameter d
...
par(bg = "aliceblue")
d <- round(d, 2)
plot(-10, xlim = c(low_x, high_x), ylim = c(0, y_max), xlab = substitute(paste("Cohen's ", delta == 0.5)), ylab = "Density", main = substitute(paste("Distribution of Cohen's ", delta == 0.5, ", N = ", 86)))
...
```{r, power-effect2, fig.margin=TRUE, echo=FALSE, fig.cap="Null and alternative distribution, assuming d = 0.8, alpha = 0.05, and N = 20 per group."}
N <- 20
d <- 0.8
p_upper <- 0.05
ncp <- (d * sqrt(N / 2)) # Calculate non-centrality parameter d
