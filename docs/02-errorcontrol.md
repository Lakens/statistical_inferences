...
\caption{Difference between Type 1 and Type 2 errors. Figure made by <a href="https://effectsizefaq.com/2010/05/31/i-always-get-confused-about-type-i-and-ii-errors-can-you-show-me-something-to-help-me-remember-the-difference/">Paul Ellis</a>}(\#fig:errortypes)
\end{figure}
...
\caption{Why there are more vaccinated people in the hospital.}(\#fig:ppvhospital)
\end{figure}
...
\caption{Screenshot of the output of the results of the PPV Shiny app by <a href="http://shinyapps.org/apps/PPV/">Michael Zehetleitner and Felix Schönbrodt </a>}(\#fig:ppvexample)
\end{figure}
...
\caption{Quote from the 1830 book by Babbage Reflections on the Decline of Science in England And on Some of Its Causes available <a href="https://www.gutenberg.org/files/1216/1216-h/1216-h.htm">here</a>}(\#fig:cooking)
\end{figure}
...
\caption{Plot of publications using CRTT (blue) and unique quantifications of the meaure (red). Figure from FlexibleMeasures.com by <a href="https://www.flexiblemeasures.com/crtt/index.php?menu=quantifications">Malte Elson</a>}(\#fig:flexiblemeasure)
\end{figure}
...
\caption{Screenshot a scientific paper explicitly admitting to using optional stopping}(\#fig:optionalstoppingexample)
\end{figure}
...
\caption[Simulated *p*-values for each additional observation when the null is true]{Simulated *p*-values for each additional observation when the null is true.}(\#fig:animatep)
\end{figure}
...
\caption[Simulated *p*-values for each additional observation when d = 0.3]{Simulated *p*-values for each additional observation when d = 0.3.}(\#fig:animatep2)
\end{figure}
...
\caption{Simulation of 500000 studies performing 5 interim analyses at an alpha level of 5%}(\#fig:optionalstopfig)
\end{figure}
...
\caption{Weighted combined error rate, minimized at alpha = 0.037.}(\#fig:minimizeerror)
\end{figure}
...
\caption{Distribution of d = 0 and d = 0.5 for an independent *t*-test with n = 50.}(\#fig:powerd)
\end{figure}
