...
<!-- Indeed, one of the most widely suggested improvements that mitigates the most important limitations of null-hypothesis significance testing is to replace the nil null hypothesis with the test of a range prediction (by specifying a non-nil null hypothesis) in an interval hypothesis test [@lakens_practical_2021]. To illustrate the difference, panel A in Figure XXXX vizualizes the results that are predicted in a two-sided null hypothesis test with a nil hypothesis, where the test examines whether an effect of 0 can be rejected. Panel B shows an interval hypothesis, where the non-nill null hypothesis consists of values smaller than 0.5 or larger than 2.5, and the interval hypothesis tests examines whether values in these ranges can be rejected.  -->
...
<!--   annotate("text", size = rel(3.5), x=0, y = plotheight-plotheight/20, parse=TRUE, label="Not~Predicted", hjust = 0.6) + #label for point null (H0) -->
...
<!--   annotate("text", size = rel(3.5), x=-1.4, y=plotheight/3, parse=TRUE, label="Predicted") + #label for lower area (H1) -->
<!--   annotate("text", size = rel(3.5), x = 1.7, y=plotheight/3, parse=TRUE, label="Predicted", hjust = 0.7) #label for upper area (H1) -->
...
<!--   annotate("text", size = rel(3.5), x=-0.8, y=plotheight/2.5, parse=TRUE, label= "NotPredicted") + #label for lower area (H1) -->
<!--   annotate("text", size = rel(3.5), x=3.8, y=plotheight/2.5, parse=TRUE, label="NotPredicted") + #label for upper area (H1) -->
<!--   annotate("text", size = rel(3.5), x=1.6, y=plotheight/2.5, parse=TRUE, label="Predicted", hjust = 0.7) #label for minimal effects area (H1) -->

<!-- grid.arrange(NHSTplot, rangeplot, ncol = 1) #combine plots in one column (all stacked) -->
...
```{r, ciequivalence, echo = FALSE, fig.cap="The mean difference and its confidence interval plotted below the the consonance density plot for tests against -0.5 and 0.5."}
res <- TOSTER::tsum_TOST(m1 = 4.55, m2 = 4.87, sd1 = 1.05, sd2 = 1.11,
