...
Within a methodological falsificationist philosophy of science, the goal of replication studies is to make claims about observational statements: By repeating a specified methodological procedure, a predicted result iss observed (or not). As De Groot [-@de_groot_methodology_1969, p. 89] writes: "If one knows something to be true, he is in a position to predict; where prediction is impossible there is no knowledge". In a direct replication study, the prediction can be as weak as 'if I repeat the procedure in the original study, the same result should be observed'. Direct replication studies are therefore not a test of a theory, and they do not tell us anything about the truth of claims. That is not their role in science. Their only function is to test whether repeating a methodological procedure will lead to a predicted observation.
...
```{r fig-alphalevels1, echo=FALSE, fig.width  = 8, fig.height = 8}
...
```{r fig-alphalevels2, echo=FALSE, fig.width  = 8, fig.height = 8}
