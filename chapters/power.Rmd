...
table_pow_justifications <- data.frame(c(
  ...
  "No justification",
  ""), 
  c("A researcher can specify the entire population, it is finite, and it is possible"
...
apa_table(table_pow_justifications, booktabs = TRUE, span_text_columns = TRUE, midrules = c(2,4,6,8,10), caption = "Overview of possible justifications for the sample size in a study.")
...
table_effect_eval <- data.frame(c(
  ...
  "in a research area"), 
  c("What is the smallest effect size that is considered theoretically or"
  , "practically interesting?", 
    "Given the test and sample size, what is the critical effect size that can",
    "be statistically significant?", 
    "Which effect size is expected based on theoretical predictions or", 
    "previous research?", 
    "Which effect sizes are excluded based on the expected width of the",
    "confidence interval around the effect size?", 
    "Across a range of possible effect sizes, which effects does a design",
    "have sufficient power to detect when performing a hypothesis test?",
    "What is the empirical range of effect sizes in a specific research area,",
    "and which effects are a priori unlikely to be observed?"))

colnames(table_effect_eval) <- c("Type of evaluation", "Which question should a researcher ask?")

apa_table(table_effect_eval, booktabs = TRUE, span_text_columns = TRUE, midrules = c(2,4,6,8,10), caption = "Overview of possible ways to evaluate which effect sizes are interesting.")
...
table_pow_rec <- data.frame(c(
  "Will a future meta-analysis",
  "be performed?", 
  "Will a decision or claim be made",
  ...
  "", 
  "",
  "Which effect sizes will a design",
  "have decent power to detect?",
  ""
  ), c(
    "Consider the plausibility that sufficient highly similar studies will be",
    "performed in the future to make a meta-analysis possible.", 
    "If a decision is made then any data that is collected will reduce error",
    "rates. Consider using a compromise power analysis to determine Type", 
    "I and Type II error rates. Are the costs worth the reduction in errors?", 
    "Report and interpret the critical effect size, with a focus on whether",
    ...
    "Report and interpret the width of the confidence interval. What will",
    "an estimate with this much uncertainty be useful for? If the null",
    ...
    "Report a sensitivity power analysis, and report the effect sizes that",
    "can be detected across a range of desired power levels (e.g., 80%, 90%, ",
...
apa_table(table_pow_rec, booktabs = TRUE, span_text_columns = TRUE, midrules = c(2,5,8,13), caption = "Overview of recommendations when reporting a sample size justification based on resource constraints.")
...
```{r, power-2, fig.height = 7, echo=FALSE, fig.cap="Power curve for an independent *t* test with an effect of *d* = 0.5 and $\\alpha$ = 0.05 as a function of the sample size."}
N <- 50
d <- 0.5
p_upper <- 0.05
...
```{r, power-3, fig.cap="Null (*d* = 0, grey dashed line) and alternative (*d* = 0.5, solid black line) hypothesis, with $\\alpha$ = 0.05 and n = 80 per group."}
...
```{r gpowprotocol, out.width = 240, echo = FALSE, fig.cap="All details about the power analysis that is performed can be exported in G*Power."}
...
table_pow_rec_2 <- data.frame(c(
  ...
  "For each power analysis, specify",
  "the effect size metric, the effect",
  "size, and the justification for",
  "powering for this effect size.", 
  "Consider the possibility that",
  "the null hypothesis is true.",
  "Make sure the power analysis ", 
  "is reproducible."), 
  c(
    "Specify all planned primary analyses that test hypotheses for which Type I",
    "and Type II error rates should be controlled.", 
    "List and justify the Type I error rate for each analysis. Make sure to correct",
    "for multiple comparisons where needed.", 
    "List and justify the desired power (or Type II error rate) for each analysis.", 
    "Report the effect size metric (e.g., Cohen's d, Cohen's f), the effect size",
    " (e.g., 0.3). and the justification for the effect size, and whether it is based",
    "is based on a smallest effect size of interest, a meta-analytic effect size",
    "estimate, the estimate of a single previous study, or some other source.",
    "Perform a power analysis for the test that is planned to examine the",
    "absence of a meaningful effect (e.g., power for an equivalence test).",
    "Include the code used to run the power analysis, or print a report",
    "containing the details about the power analyses that has been performed."))
colnames(table_pow_rec_2) <- c("What to take into account?", "How to take it into account?")

apa_table(table_pow_rec_2, booktabs = TRUE, span_text_columns = TRUE, midrules = c(2,4,5,9,11), caption = "Overview of recommendations when reporting an a-priori power analysis.")
```
...
```{r, power-effect1, fig.margin=FALSE, echo=FALSE, fig.cap="Critical effect size for an independent *t* test with n = 15 per group and $\\alpha$ = 0.05."}
N <- 15
d <- 0.5
p_upper <- 0.05
ncp <- (d * sqrt(N / 2)) # Calculate non-centrality parameter d
...
```{r gcrit2, fig.margin=FALSE, out.width = 240, fig.cap="The critical correlation of a test based on a total sample size of 30 and $\\alpha$ = 0.05 calculated in G*Power."}
...
table_meta_just <- data.frame(c(
  ...
  "meta-analysis similar?",
  "",
  "Are the studies in the",
  "meta-analysis homogeneous?", 
  "Is the effect size estimate",
  "unbiased?",
  "",
  "",
  ""), 
  c("Are the studies in the meta-analyses very similar in design, measures, and",
  "the population to the study you are planning? Evaluate the generalizability",
  "of the effect size estimate to your study.", 
  "Is there heterogeneity in the meta-analysis? If so, use the meta-analytic",
  "effect size estimate of the most relevant homogenous subsample.",
  "Did the original study report bias detection tests, and was there bias?",
  "If so, it might be wise to use a more conservative effect size estimate,",
...
apa_table(table_meta_just, booktabs = TRUE, span_text_columns = TRUE, midrules = c(3,5), caption = "Overview of recommendations when justifying the use of a meta-analytic effect size estimate for a power analysis.")
...
```{r follow-up-bias, fig.cap="Distribution of partial eta squared under the null hypothesis (dotted grey curve) and a medium true effect of 0.0588 (solid black curve) for 3 groups with 25 observations.", echo = FALSE, cache = TRUE}
N <- 25
J <- 3
eta_pop <- 0.0588
alpha_level <- 0.05
...
table_es_just <- data.frame(c(
  ...
  "",
  ""), 
  c("Consider if there are differences between the studies in terms of the",
  ...
  "Evaluate the possibility that if the effect size estimate had been smaller, ",
  ...
  "Studies with a small number of observations have large uncertainty.",
...
apa_table(table_es_just, booktabs = TRUE, span_text_columns = TRUE, midrules = c(3,7), caption = "Overview of recommendations when justifying the use of an effect size estimate from a single study.")
```
...
res <- d.ind.t(m1 = 0, m2 = 0, sd1 = 4, sd2 = 4, 
               n1 = 15, n2 = 15, a = .05)
# Print the result
# res$estimate
...
```{r gsens0, out.width = 240, fig.cap="Sensitivity power analysis in G*Power software."}
...
```{r gpowcompromise, out.width = 240, echo = FALSE, fig.cap="Compromise power analysis in G*Power."}
...
table_compromise <- data.frame(c(
  ...
  "What is the justification",
  "for the effect size?",
  "What is the desired ratio of", 
  "Type I vs Type II error rates?"), 
  c("Specify why a specific sample size is collected (e.g., based on resource",
  "constraints or other factors that determined the sample size).", 
  "Is the effect size based on a smallest effect size of interest or an",
  "expected effect size?",
  "Weigh the relative costs of a Type I and a Type II error by carefully",
  "evaluating the consequences of each type of error."))

colnames(table_compromise) <- c("What to take into account", "How to take it into account?")

apa_table(table_compromise, booktabs = TRUE, span_text_columns = TRUE, midrules = c(2,4,6), caption = "Overview of recommendations when justifying error rates based on a compromise power analysis.")
```
...
```{r obs-power-plot-2, echo = FALSE, fig.width = 7, fig.height = 7, fig.cap="Relationship between *p* values and power for an independent *t* test with $\\alpha$ = 0.05 and n = 10."}
...
```{r, plot-1, fig.margin=FALSE, echo=FALSE, fig.height = 2.8, fig.fullwidth = TRUE, fig.cap="Distributions of two dependent groups with means 100 and 106 and a standard deviation of 15, distribution of the differences, and correlation of 0."}
set.seed(419)
#Set color palette
cbbPalette <- c("#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")
...
```{r, plot-4, fig.margin=FALSE, echo=FALSE, fig.height = 2.8, fig.fullwidth = TRUE, fig.cap="Distributions of two independent groups with means 100 and 106 and a standard deviation of 15, distribution of the differences, and correlation of 0.7."}
set.seed(419)
#Set color palette
cbbPalette<-c("#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")
