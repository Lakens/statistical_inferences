# Sample Size Justification {#sec-power}
...
*You can listen to an audio recording of this chapter [here](https://soundcloud.com/lakens/sample-size-justification-by-daniel-lakens). You can download a translation of this chapter in Chinese [here](https://surfdrive.surf.nl/files/index.php/s/vLD38Wf7QqfeYqc)*
...
#| tbl-cap: "Overview of possible justifications for the sample size in a study."
...
#| tbl-cap: "Overview of possible ways to evaluate which effect sizes are interesting."
...
# table_print <- knitr::kable(table_effect_eval, "html", booktabs = T) 
# 
# table_print <- kableExtra::column_spec(table_print, 1, width = "5cm")
# table_print <- kableExtra::column_spec(table_print, 2, width = "10cm")
# table_print
df_2_MD(table_effect_eval)
...
# table_print <- knitr::kable(table_pow_rec, "html", booktabs = T) 
# 
# table_print <- kableExtra::column_spec(table_print, 1, width = "5cm")
# table_print <- kableExtra::column_spec(table_print, 2, width = "10cm")
# table_print
df_2_MD(table_pow_rec)

```

## A-priori Power Analysis{#sec-aprioripower}
...
Given a specified effect size, alpha level, and power, an a-priori power analysis can be used to calculate the number of observations required to achieve the desired error rates, given the effect size. Power analyses can be performed based on standardized effect sizes or effect sizes expressed on the original scale. It is important to know the standard deviation of the effect (see the 'Know Your Measure' section) but I find it slightly more convenient to talk about standardized effects in the context of sample size justifications. @fig-power-2 illustrates how the statistical power increases as the number of observations (per group) increases in an independent *t* test with a two-sided alpha level of 0.05. If we are interested in detecting an effect of *d* = 0.5, a sample size of 90 per condition would give us more than 90% power. Statistical power can be computed to determine the number of participants, or the number of items [@westfall_statistical_2014] but can also be performed for single case studies [@ferron_power_1996; @mcintosh_power_2021].
...
#| fig-cap: "Power curve for an independent *t* test with an effect of *d* = 0.5 and alpha = 0.05 as a function of the sample size."
...
#| fig-cap: "All details about the power analysis that is performed can be exported in G\\*Power."

knitr::include_graphics("images/gpowprotocol.png")
```

The reproducible report needs to be accompanied by justifications for the choices that were made with respect to the values used in the power analysis. If the effect size used in the power analysis is based on previous research, the factors presented in @tbl-tablemetajust (if the effect size is based on a meta-analysis) or @tbl-table-es-just (if the effect size is based on a single study) should be discussed. If an effect size estimate is based on the existing literature, provide a full citation, and preferably a direct quote from the article where the effect size estimate is reported. If the effect size is based on a smallest effect size of interest, this value should not just be stated, but justified (e.g., based on theoretical predictions or practical implications, see @lakens_equivalence_2018). For an overview of all aspects that should be reported when describing an a-priori power analysis, see @tbl-table-pow-rec-2.

```{r tbl-table-pow-rec-2, echo = FALSE, results = 'asis'}
#| tbl-cap: "Overview of recommendations when reporting an a-priori power analysis."
...
# table_print <- knitr::kable(table_pow_rec_2, "html", booktabs = T) 
# 
# table_print <- kableExtra::column_spec(table_print, 1, width = "5cm")
# table_print <- kableExtra::column_spec(table_print, 2, width = "10cm")
# table_print
df_2_MD(table_pow_rec_2)
...
If researchers do not want to test a hypothesis, for example because they prefer an estimation approach over a testing approach, then in the absence of clear guidelines that help researchers to justify a desired level of precision, one solution might be to rely on a generally accepted norm of precision. This norm could be based on ideas about a certain resolution below which measurements in a research area no longer lead to noticeably different inferences. Just as researchers normatively use an alpha level of 0.05, they could plan studies to achieve a desired confidence interval width around the observed effect that is determined by a norm. Future work is needed to help researchers choose a confidence interval width when planning for accuracy (see also the section on which confidence interval to use in Bayesian tests of [range predictions](#sec-whichinterval)).

In the interactive visualization below, you can explore the required sample size to precisely estimate a correlation. First, take a moment to think about which level of accuracy you think is sufficient if you would estimate a correlation. If the true correlation is 0.3, how wide would you prefer the confidence interval to be? If data is collected from 50 observations, the confidence interval would range from *r* = 0.024 to *r* = 0.534. Would the informational value provided by this sample size be high enough to warrant data collection? If all stakeholders would already strongly believe that the true correlation falls within this range, collecting the data would not tell the stakeholders anything they didn't already know. Which confidence interval width would increase our knowledge about the correlation? Change the setting to Accuracy -> N, and set a desired confidence interval width (for example, 0.2, which means the 95% confidence interval would range from *r* = 0.197 to *r* = 0.397). This range is still somewhat wide, but would probably increase our knowledge about the population value above what stakeholders expected without any data. Explore which sample sizes are needed to reduce the confidence interval width even further, to a range narrow enough that you would find the estimate highly informative. Also explore which confidence intervals you can expect when the true correlation is close to the lower and upper bound of the possible range (i.e., -1 and 1, respectively).
...
<iframe id="prec-cor-iframe"
        ...
        title="Planning for Accuracy – Correlation">
</iframe>
<script>
...
An interactive version of this simulation is available in the online edition of this textbook.
:::
...
## The Minimal Statistically Detectable Effect {#sec-minimaldetectable2}

The [minimal statistically detectable effect](#sec-minimaldetectable1) is the smallest effect size that, if observed, would yield a statistically significant *p*-value [@cook_assessing_2014]. In @fig-power-effect1, the distribution of Cohen’s *d* is plotted for 15 participants per group when the true effect size is either *d* = 0 or *d* = 0.5. This figure is similar to @fig-power-3, with the addition that the critical *d* is indicated. We see that with such a small number of observations in each group only observed effects larger than *d* = 0.75 will be statistically significant. Whether such effect sizes are interesting, and can realistically be expected, should be carefully considered and justified.

```{r fig-power-effect1, fig.margin=FALSE, echo=FALSE}
#| fig-cap: "Critical effect size for an independent *t* test with n = 15 per group and $\\alpha$ = 0.05."
...
```{r tbl-tablemetajust, echo = FALSE, results = 'asis'}
#| tbl-cap: "Overview of recommendations when justifying the use of a meta-analytic effect size estimate for a power analysis."
...
The two main reasons researchers should be careful when using effect sizes from studies in the published literature in power analyses is that effect size estimates from studies can differ from the true population effect size due to random variation, and that publication bias inflates effect sizes. @fig-followupbias shows the distribution for $\eta_p^2$ for a study with three conditions with 25 participants in each condition when the null hypothesis is true (dotted grey curve) and when there is a 'medium' true effect of $\eta_p^2$ = 0.0588 [solid black curve; @richardson_eta_2011]. As in @fig-power-effect1 the critical effect size is indicated, which shows observed effects smaller than $\eta_p^2$ = 0.08 will not be significant with the given sample size. If the null hypothesis is true, effects larger than $\eta_p^2$ = 0.08 will be a Type I error (the dark grey area), and when the alternative hypothesis is true effects smaller than $\eta_p^2$ = 0.08 will be a Type II error (light grey area). It is clear all significant effects are larger than the true effect size ($\eta_p^2$ = 0.0588), so power analyses based on a significant finding (e.g., because only significant results are published in the literature) will be based on an overestimate of the true effect size, introducing bias. 

But even if we had access to all effect sizes (e.g., from pilot studies you have performed yourself) due to random variation the observed effect size will sometimes be quite small. @fig-followupbias shows it is quite likely to observe an effect of $\eta_p^2$ = 0.01 in a small pilot study, even when the true effect size is 0.0588. Entering an effect size estimate of $\eta_p^2$ = 0.01 in an a-priori power analysis would suggest a total sample size of 957 observations to achieve 80% power in a follow-up study. If researchers only follow up on pilot studies when they observe an effect size in the pilot study that, when entered into a power analysis, yields a sample size that is feasible to collect for the follow-up study, these effect size estimates will be upwardly biased, and power in the follow-up study will be systematically lower than desired [@albers_when_2018]. 


```{r fig-followupbias, echo = FALSE}
#| fig-cap: "Distribution of partial eta squared under the null hypothesis (dotted grey curve) and a medium true effect of 0.0588 (solid black curve) for 3 groups with 25 observations."
N <- 25
J <- 3
eta_pop <- 0.0588
alpha_level <- 0.05
...
To summarize, an effect size from a previous study in an a-priori power analysis can be used if three conditions are met (see @tbl-table-es-just). First, the previous study is sufficiently similar to the planned study. Second, there was a low risk of bias (e.g., the effect size estimate comes from a Registered Report, or from an analysis for which results would not have impacted the likelihood of publication). Third, the sample size is large enough to yield a relatively accurate effect size estimate, based on the width of a 95% CI around the observed effect size estimate. There is always uncertainty around the effect size estimate, and entering the upper and lower limit of the 95% CI around the effect size estimate might be informative about the consequences of the uncertainty in the effect size estimate for an a-priori power analysis.

```{r tbl-table-es-just, echo = FALSE, results = 'asis'}
#| tbl-cap: "Overview of recommendations when justifying the use of an effect size estimate from a single study."
...
# table_print <- knitr::kable(table_es_just, "html", booktabs = T) 
# 
# table_print <- kableExtra::column_spec(table_print, 1, width = "5cm")
# table_print <- kableExtra::column_spec(table_print, 2, width = "10cm")
# table_print
df_2_MD(table_es_just)
```
...
res <- MOTE::d_ind_t(m1 = 0, m2 = 0, sd1 = 4, sd2 = 4, 
                     n1 = 15, n2 = 15, a = .05)
# Print the result
# res$estimate
...
res2 <- MOTE::d_ind_t(m1 = 0.5, m2 = 0, sd1 = 1, sd2 = 1, n1 = 15, n2 = 15, a = .05)

res3 <- MOTE::d_ind_t(m1 = 0.7479725, m2 = 0, sd1 = 1, sd2 = 1, n1 = 15, n2 = 15, a = .05)
...
# table_print <- knitr::kable(table_compromise, "html", booktabs = T) 
# 
# table_print <- kableExtra::column_spec(table_print, 1, width = "5cm")
# table_print <- kableExtra::column_spec(table_print, 2, width = "10cm")
# table_print
df_2_MD(table_compromise)

```

## What to do if Your Editor Asks for Post-hoc Power?{#sec-posthocpower}
...
For other statistical tests, where the alternative distribution is not symmetric (such as for the *t* test, where the alternative hypothesis follows a non-central *t* distribution, see @fig-noncentralt), a *p* = 0.05 does not directly translate to an observed power of 50%, but by plotting post-hoc power against the observed *p* value we see that the two statistics are always directly related. As @fig-obs-power-plot-2 shows, if the *p* value is non-significant (i.e., larger than 0.05) the observed power will be less than approximately 50% in a *t* test. Lenth [-@lenth_post_2007] explains how observed power is also completely determined by the observed *p* value for *F* tests, although the statement that a non-significant *p* value implies a power less than 50% no longer holds. 

```{r fig-obs-power-plot-2, echo = FALSE, fig.width = 7, fig.height = 7}
#| fig-cap: "Relationship between *p* values and power for an independent *t* test with alpha = 0.05 and n = 10."
...
p1 <- ggplot(datasetplot, aes(DV, fill = as.factor(IV))) +
  ...
  labs(subtitle = paste("Mean X =", round(mean(x)), "(SD =", round(sd(x)), ")\nMean Y =", round(mean(y)), "(SD =", round(sd(y)), ")")) +
...
p2 <- ggplot(as.data.frame(dif), aes(dif)) +
  ...
  labs(subtitle = paste("Mean =", round(mean(dif)), "\nSD =", round(sd(dif)))) +
...
p4 <- ggplot(datasetplot, aes(DV, fill = as.factor(IV)))  + 
  ...
  stat_function(fun=function(z) dnorm(z, mean=mx, sd=sdx) * n * 2, linewidth=1, color="#E69F00", lty=2) +
  stat_function(fun=function(z) dnorm(z, mean=my, sd=sdy) * n * 2, linewidth=1, color="#56B4E9", lty=2) +
  xlab("IQ") + ylab("Count")  + theme_bw(base_size=12) + 
  theme(plot.background = element_rect(fill = backgroundcolor))  + 
  theme(panel.background = element_rect(fill = backgroundcolor)) + 
...
p5 <- ggplot(as.data.frame(dif), aes(dif))  + 
  geom_histogram(colour="black", fill="grey", binwidth=2) +
  #  geom_density(fill=NA, colour="black", size = 1) +
  xlab("IQ dif") + ylab("Count")  + theme_bw(base_size=10) + 
  theme(plot.background = element_rect(fill = backgroundcolor))  + 
  theme(panel.background = element_rect(fill = backgroundcolor)) + 
  theme(plot.subtitle = element_text(hjust = 0.5, size = 8)) +
  theme(panel.grid.major.x = element_blank(), axis.text.y = element_blank(), panel.grid.minor.x = element_blank()) + 
...
p6 <- ggplot(dataset, aes(x=x, y=y)) +
  geom_point(size=2) +    # Use hollow circles
  geom_smooth(formula = y ~ x, method=lm, colour="#E69F00", size = 1, fill = "#56B4E9") + # Add linear regression line
  coord_fixed(ratio = 1, xlim=c(40,160), ylim=c(40,160)) +
