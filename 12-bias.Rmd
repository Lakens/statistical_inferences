...
```
BACKGROUNDCOLOR ISSUE AGAIN IN THE ABOVE CODE.
...
An alternative technique that also meta-analyzes the *p*-values from individual studies is the *z*-curve, which is a meta-analysis of observed power ([@bartos_z-curve20_2020; @brunner_estimating_2020]; for an example, see [@sotola_garbage_2022]). Like a traditional meta-analysis, *z*-curves transform observed test results (*p*-values) into *z*-scores. In an unbiased literature where the null hypothesis is true, we should observe approximately $\alpha$% significant results. If the null is true, the distribution of *z*-scores is centered on 0. The *z*-curve computes absolute *z*-values, and therefore $\alpha$% of *z*-scores should be larger than the critical value (1.96 for a 5% alpha level). In Figure \@ref(fig:zcurveunbiasednull) *z*-scores for 1000 studies are plotted, with a true effect size of 0, where exactly 5% of the observed results are statistically significant. 
