...
*An interactive simulation of publication bias in meta-analysis is available in the online version of this book.*
...
sim_sig <- function(n_sig) {
  ...
  for (i in seq_len(n_sig)) {
    ...
    p <- 1
    while (p > 0.025) {
      x <- rnorm(n, mean = m1, sd = sd1); y <- rnorm(n, mean = m2, sd = sd2)
      p <- t.test(x, y, alternative = "greater", var.equal = TRUE)$p.value
    }
    res <- t.test(x, y, var.equal = TRUE)
    out[i, ] <- list(mean(x), mean(y), sd(x), sd(y), n, n, res$p.value,
...
```{r fig-zcurveunbiasedalternative, echo = FALSE, cache = TRUE, warning = FALSE}
...
```{r fig-zcurveq6, cache = TRUE, echo = FALSE, warning = FALSE}
...
opts_p <- c(
  ...
  answer = "The trim-and-fill analysis would indicate bias, but so did the *p*-curve and *z*-curve analysis, and the adjusted effect size estimate by trim-and-fill does not adequately correct for bias, so the analysis would not add anything.", 
