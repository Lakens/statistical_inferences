...
In other words, both the null hypothesis test and the equivalence test have yielded significant results. This means we can claim that the observed effect is statistically different from zero, and that the effect is statistically smaller than effects we deemed large enough to matter when we specified the equivalence range from -0.5 to 0.5. This illustrates how combining equivalence tests and nil null hypothesis tests can prevent us from mistaking statistically significant effects for practically significant effects. In this case, with 200 participants, we can reject an effect of 0, but the effect, if any, is not large enough to be meaningful. In the interactive calculator below you can perform an equivalence test for an independent *t*-test by specifying the means, standard deviations, sample sizes, and bounds. Examine the test results for different settings, such as large sample sizes, small sample sizes, a large true effect, and a null effect.
...
<iframe id="toster-ttest-iframe"
        ...
        title="Equivalence Test (TOST) for an Independent t-test">
</iframe>
<script>
...
*An interactive equivalence test calculator for an independent t-test is available in the online version of this book.*
...
Statistical software offers options for power analyses for some statistical tests, but not for all tests. Just as with power analysis for a nil null hypothesis test, it can be necessary to use a simulation-based approach to power analysis. In the interactive calculator below you can perform a priori power analyses for an independent *t*-test. Explore the effects on the required sample size of a wider or more narrow bound, tests again a null effect (a true difference of 0) or a non nill null (e.g., a small true difference) and a higher or lower desired statistical power.
...
<iframe id="toster-power-iframe"
        ...
        scrolling="no"
        style="border:none;display:block;margin:1.5rem 0;overflow:hidden;"
        title="Power Analysis for an Equivalence Test (Independent t-test)">
</iframe>
<script>
window.addEventListener('message', function(e) {
  if (e.data && typeof e.data.iframeHeight === 'number') {
...
*An interactive power analysis calculator for an equivalence test for an independent t-test is available in the online version of this book.*
