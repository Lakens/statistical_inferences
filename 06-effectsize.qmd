...
#| fig-cap: "Figure from Lyons et al (2015) platting raw mean differences (left), variance (middle), and standardized effect size (right)."
...
Estimates based on samples from the population will show variability. The larger the sample, the closer our estimates will be to the true population values, as explained in the next chapter on [confidence intervals](#sec-confint). Sometimes we will observe larger estimates than the population value, and sometimes we will observe smaller values. As long as we have an unbiased collection of effect size estimates we can combine them in a [meta-analysis](#sec-meta) to increase the accuracy of the estimate. Regrettably, the scientific literature is often biased. It is specifically common that statistically significant studies are published (e.g., studies with *p*-values smaller than 0.05) and studies with *p*-values larger than 0.05 remain unpublished. Instead of having access to all effect sizes, anyone reading the literature only has access to effects that passed a **significance filter**. 

If the observed *p*-value is 1, the observed effect size is 0. All else equal, the lower the *p*-value, the larger the effect size. If we only report studies with *p*-values below 0.05, then we only report studies with relatively larger effect sizes. This means that on average, effect size estimates from a biased literature with a significance filter will suffer from **inflated effect sizes**. 

Another way to think about this is through the concept of a **truncated distribution**. If effect sizes are only reported if the *p*-value is statistically significant, then we only have access to effect sizes that are larger than some minimal value [@taylor_bias_1996; @anderson_sample-size_2017]. In @fig-inflated only effects larger than d = 0.4 can be significant, so all effect sizes below this threshold are censored, and only effect sizes in the gray part of the distribution will be available to researchers. Without the lower part of the effect size distribution effect sizes will on average be inflated. 
