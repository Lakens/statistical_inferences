...
```{r, ppvhospital, echo = FALSE, fig.cap="Why there are more vaccinated people in the hospital."}
...
```{r, cooking, echo = FALSE, fig.cap='Quote from the 1830 book by Babbage Reflections on the Decline of Science in England And on Some of Its Causes available <a href="https://www.gutenberg.org/files/1216/1216-h/1216-h.htm">here</a>'}
...
```{r, flexiblemeasure, echo = FALSE, fig.cap='Plot of publications using CRTT (blue) and unique quantifications of the meaure (red). Figure from FlexibleMeasures.com by <a href="https://www.flexiblemeasures.com/crtt/index.php?menu=quantifications">Malte Elson</a>'}
...
```{r optionalstoppingexample, echo = FALSE, fig.cap="Screenshot a scientific paper explicitly admitting to using optional stopping"}
...
```{r, animatep-gif, eval=FALSE, echo=FALSE, cache=TRUE}
set.seed(929)
...
```{r animatep, echo = FALSE, fig.cap='Simulated *p*-values for each additional observation when the null is true.'}
...
```{r animatep2-gif, eval=FALSE, echo=FALSE, cache=TRUE}
set.seed(928)
...
# save last image for PDF
file.copy(filenames[n], "images/animatep2.png")

# make gif for html
gifski::gifski(png_files = filenames, 
               gif_file = "images/animatep2.gif", 
               delay = 0.01)

```

```{r animatep2, echo = FALSE, fig.cap='Simulated *p*-values for each additional observation when d = 0.3.'}
if (knitr::is_latex_output()) {
...
```{r, optionalstopfig, eval=TRUE, echo=FALSE, cache=TRUE, fig.cap='Simulation of 500000 studies performing 5 interim analyses at an alpha level of 5%'}
N <- 100 # total datapoints (per condition)
looks <- 5 # set number of looks at the data
...
```{r, powerd, eval=TRUE, echo=FALSE, cache=TRUE, fig.cap='Distribution of d = 0 and d = 0.5 for an independent *t*-test with n = 50.'}
N <- 50
d <- 0.5
alpha_level <- 0.05
