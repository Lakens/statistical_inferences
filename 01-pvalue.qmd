...
# Better BibTeX citation key (Zotero 9, one-word short title): auth.lower + "_" + shorttitle(1, 1).lower + "_" + year
...
Assume I ask two groups of 10 people how much they liked the extended director's cut of the Lord of the Rings (LOTR) trilogy (see @tbl-friends). This means our **total sample size** (*N*) is 20, and the sample size in each group (*n*) is 10. The first group consists of my friends, and the second groups consists of friends of my wife. Our friends rate the trilogy on a score from 1 to 10. We can calculate the average rating by my friends, which is 8.7, and the average rating by my wife’s friends, which is 7.7. We can compare the scores in both groups by looking at the raw data, and by plotting the data.

```{r tbl-friends, echo=F}
#| tbl-cap: "Ratings for the Lord of the Rings extended trilogy by two groups of friends."
...
Which *p*-values you can expect is completely determined by the statistical power of the study, or in other words the probability that you will observe a significant effect if there is a true effect. The statistical power ranges from 0 to 1. We can illustrate this by simulating independent *t*-tests. The idea is that we simulate reaction times for two groups of people. We know the standard deviation of reaction times in this task is 75 ms. We will set the mean reaction time in one simulated group to 100 ms, and in the other simulated group to 130 ms. We are testing whether people in one group have a different reaction time from the other group (and we know the correct answer is 'yes', because we made it so in the simulation).

The interactive visualization below runs this simulation. In each simulated experiment, *n* = 49 reaction times are drawn from a normal distribution for each group (by default: *M* = 100 ms and *M* = 130 ms, SD = 75 ms), an independent *t*-test is performed, and the *p*-value is stored. After all simulations are complete, the distribution of *p*-values is plotted as a histogram. Use the inputs to change the group means, the standard deviation, the sample size, the number of simulations, and the alpha level, then click **Run simulation**.
...
<iframe id="pval-sim-iframe"
        ...
        title="P-value Distribution Simulation">
</iframe>
<script>
...
*An interactive p-value distribution simulation is available in the online version of this book.*

:::

On the x-axis we see *p*-values from 0 to 1 in 20 bars, and on the y-axis we see how frequently these *p*-values were observed. The red dotted horizontal line shows the expected frequency per bar if H₀ were true — with 100,000 simulations and 20 bars, that is 5,000 per bar. In the title of the simulation, the statistical power achieved in the simulated studies is given (assuming α = 0.05). With the default settings (*n* = 49, *M* = 100 ms vs *M* = 130 ms, SD = 75 ms), the studies have approximately 50% power.

The simulation result illustrates the **probability density function** of *p*-values for a *t*-test. A probability density function provides the probability that a random variable has a specific value (such as @fig-tdist) of the *t*-distribution). Because the *p*-value is a random variable, we can use its probability density function to plot the *p*-value distribution [@hung_behavior_1997; @ulrich_properties_2018], as in the interactive figure below. The *p*-value distribution is a function of the statistical power of the test (which is higher if the sample size, effect size, or alpha level of the test are increased). You can explore this by increasing or decreasing the power by moving the slider in the interactive visualization below.
...
*An interactive p-value distribution app is available in the online version of this book.*

:::

When there is no true effect, and test assumptions are met, *p*-values for a *t*-test are **uniformly distributed**. This means that every *p*-value is equally likely to be observed when the null hypothesis is true. In other words, when there is no true effect, a *p*-value of 0.08 is just as likely as a *p*-value of 0.98. I remember thinking this was very counterintuitive when I first learned about uniform *p*-value distributions (after completing my PhD). But it makes sense that *p*-values are uniformly distributed when we remember that, when $H_0$ is true, alpha % of the *p*-values should fall below the alpha level, whichever alpha level we set. Hence, if we set alpha to 0.01, 1% of the observed *p*-values should fall below 0.01, and if we set alpha to 0.12, 12% of the observed *p*-values should fall below 0.12. This can only happen if *p*-values are uniformly distributed when the null hypothesis is true. Note that the uniform *p*-value distribution only emerges if the distribution of the test statistic is continuous (e.g., such as for the *t*-test), and not if the distribution of the test statistic is discrete (e.g., in the case of the chi-squared test see @wang_pvalue_2019). You can examine this behavior by setting the means of both groups to the same value, or the power to 5%, in the interactive simulations above.
