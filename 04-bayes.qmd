...
*An interactive Beta distribution visualization is available in the online version of this book.*
...
The interactive visualization below shows how each prior is updated to a posterior after observing the data. In each panel the grey line is the prior, the blue dashed line is the likelihood, and the red line is the posterior. Use the n (flips) and x (heads) inputs to see how the prior, likelihood, and posterior shift with different data. The default is n = 20, x = 10. Note that the Strong Believer uses a Beta(10, 0.5) prior — strong enough that with only 20 flips the posterior remains biased toward heads even when exactly 50% heads are observed.
...
*An interactive Bayesian updating visualization is available in the online version of this book.*

:::

For the strong believer the posterior is pulled noticeably toward the prior: the data show 50% heads, but the posterior still leans toward heads because the prior carries substantial weight (Beta(10, 0.5) is equivalent to having previously seen around 9 extra heads with little counter-evidence). The slight skeptic and the strong skeptic end up with a much stronger belief in a fair coin after observing the data than the newborn and the strong believer, but primarily because they already started with a stronger prior that the coin was fair.

## Updating our belief

Now that we have a distribution for the prior, and a distribution for the posterior, we can compare where the posterior is higher than the prior to see for which values of *p* our belief has increased. Everywhere where the posterior (red) is higher than the prior (grey), our belief in that value of *p* has increased.

The Bayes Factor quantifies this change in relative evidence at a specific point null $H_0$. It is simply the value of the posterior distribution at $H_0$ divided by the value of the prior distribution at $H_0$ — a ratio known as the **Savage-Dickey density ratio**. For the newborn baby's prior of Beta(1, 1) and 10 heads out of 20 flips, where $H_0$: *p* = 0.5:

$$BF_{10} = \frac{\text{Beta}(p = 0.5,\; 11,\; 11)}{\text{Beta}(p = 0.5,\; 1,\; 1)} = \frac{3.70}{1} = 3.70$$

Use the interactive calculator below to explore how the Bayes Factor changes with different priors and data. The grey line is the prior, the blue dashed line is the likelihood (which is only visible if you change the settings from a uniform to an informed prior), and the red line is the posterior. The grey dot marks the prior density at $H_0$, the red dot marks the posterior density at $H_0$, and the dashed segment between them illustrates the ratio that gives $BF_{10}$.
...
*An interactive Bayes Factor calculator is available in the online version of this book.*
...
Use the Bayes Factor calculator above (with default settings: $H_0$ = 0.5, *n* = 20, *x* = 10, $\alpha$ = 1, $\beta$ = 1). The calculator plots the prior (grey), likelihood (blue dashed), and posterior (red), and shows $BF_{10}$ as the ratio of the posterior to prior density at $H_0$.

**Q3**: In the calculator, change $H_0$ to 0.675 (keep everything else at default). If you were testing the idea that this coin returns 67.5% heads, which statement is true?
...
**Q4**: Set $H_0$ back to 0.5 in the calculator. Now set $\alpha$ = 100 and $\beta$ = 100 (keeping *n* = 20, *x* = 10) to examine the extreme skeptic's belief update. Compare $BF_{10}$ to the newborn's $BF_{10}$ = 3.70. Which statement is true?
...
opts_p <- c(
   ...
   "The frequentist mean is **higher** than the mean of the posterior, because by combining the prior with the data, the mean of the posterior is **further from** the mean of the prior distribution.", 
