...
```{r friends, echo=F, tab.cap = "Ratings for the Lord of the Rings extended trilogy by two groups of friends.", ft.align="left"}
...
df <- flextable(df)
df <- autofit(df)
df <- set_header_labels(df, friends = "", friends_daniel = "Friends Daniel", friends_kyra = "Friends Kyra")
theme_booktabs(df)
```
...
df_long <- tidyr::gather(df, daniel_kyra, rating, friends_daniel:friends_kyra, factor_key = TRUE)

# Plot the data
ggplot(df_long, aes(x = rating, fill = daniel_kyra)) +
...
```{r, echo = FALSE, tab.cap = "Summary statistics for our dataset.", ft.align="left"}
sum_table <- summarise(group_by(df_long, daniel_kyra),
                       mean = mean(rating), sd = sd(rating), n = n(), 
                       .groups = 'drop')
sum_table <- flextable(sum_table)
sum_table <- autofit(sum_table)
theme_booktabs(sum_table)
```
...
If you want to explore *p*-value distributions using the simulation code above, you can do so in [this shiny app](http://shiny.ieis.tue.nl/pvalues/) below. 
