...
In the simulation, we generate n = 71 normally distributed IQ scores with a mean of M (105 by default) and a standard deviation of 15. We then perform a one-sample *t*-test, store the *p*-value, and generate a plot of the *p*-value distribution.   IS IT REALLY A ONE-SAMPLE T TEST THAT IS PERFORMED HERE?
...
#Plot figure
op <- par(mar = c(5,7,4,4)) #change white-space around graph
par(bg = backgroundcolor) # THROUGHOUT THIS CHAPTER, AGAIN THE BACKGROUNDCOLOR LINES OF CODE ARE NOT WORKING (FOR ME).
hist(p, breaks=bars, xlab="P-values", ylab="number of p-values\n", axes=FALSE,
...
FOR THE ABOVE CODE, IS THERE A PACKAGE USED? (FOR calc_d_dist AND FOR add_type1_error).
...
```
FOR ME, I THINK THE FIGURE CAPTION COULD SAY MORE HERE. FOR EXAMPLE, WHAT IS MEANT BY THE T VALUE ANNOTATION WITHIN THE FIGURE?
