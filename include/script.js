...
function resolve_unresolved_xrefs() {
  var links = document.querySelectorAll("a.quarto-xref");
  ...
  for (var i = 0; i < links.length; i++) {
    var link = links[i];
    var unresolved = link.querySelector("span.quarto-unresolved-ref");
    if (!unresolved) {
      continue;
    }
    ...
    var caption = target.querySelector("figcaption.quarto-float-caption");
    if (!caption) {
      continue;
    }

    var caption_text = (caption.textContent || "").trim();
    if (!caption_text) {
      continue;
    }

    var num_match = caption_text.match(/^(Figure|Table|Equation|Listing|Lemma|Theorem|Corollary|Proposition)\s+([0-9]+(?:\.[0-9]+)*)\s*:/i);
    if (num_match && num_match[2]) {
      unresolved.textContent = num_match[2];
    }
  }
}

add_accessibility_controls();
resolve_unresolved_xrefs();
</script>